...
// console.log(buttonContainer)
  ...
  button.textContent = video.name;
      ...
      //   showContent('showButton1');
      // };
    });
    const firstButtonId = document.querySelector('#showButton1').dataset.id;
    fetchFilmDataAndPopulate(firstButtonId);
})
  .catch((error) => {
...
function fetchFilmDataAndPopulate(id) {
  fetch(`https://indiefilmsjson-1nbcwi81c-treneeses-projects.vercel.app/${id}`)
    ...
    .then((video) => {
      ...
      document.querySelector('.name').textContent = video.name;
      document.querySelector('.about').textContent = video.about;
      document.querySelector('.director').textContent = video.director;
      document.querySelector('.writer').textContent = video.writers;

      document.querySelector('.cast').textContent = video['cast/crew'];
...
// // Call fetchFilmDataAndPopulate with ID of showButton1 when the page loads
window.onload = function() {
  fetchFilmDataAndPopulate(document.querySelector('#showButton1').dataset.id);
};
...
// function fetchFilmDataAndPopulate(id) {
//   // Your existing code to fetch and populate film data
// }
...
//       // Call fetchFilmDataAndPopulate with ID of showButton1 as default
//       fetchFilmDataAndPopulate(document.querySelector('#showButton1').dataset.id);
  // })
  // .catch((error) => {
  //     console.error('Error fetching films:', error);
